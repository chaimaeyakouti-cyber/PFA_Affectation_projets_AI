...
export default function EtudiantDashboard() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: palette.bg, fontFamily: "Inter, system-ui, sans-serif" }}>
      <style>{`
        * { box-sizing: border-box; }
        .nav-card { transition: transform 0.2s ease, box-shadow 0.2s ease; cursor: pointer; }
        .nav-card:hover { transform: translateY(-4px); box-shadow: 0 12px 40px rgba(8,145,178,0.15); }
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 72 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div>
              ...
              <div style={{ color: palette.border, fontSize: 12, fontFamily: "Inter, system-ui, sans-serif", fontWeight: 300 }}>INPT · Plateforme de gestion de projets</div>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              <div style={{ color: '#fff', fontSize: 13, fontFamily: "Inter, system-ui, sans-serif" }}>{currentUser?.nom || 'Étudiant'}</div>
              <div style={{ color: '#A5F3FC', fontSize: 11, fontFamily: "Inter, system-ui, sans-serif" }}>{currentUser?.email}</div>
            </div>
            <button onClick={() => { localStorage.removeItem('user'); localStorage.removeItem('access_token'); navigate('/') }} style={{ background: 'rgba(255,255,255,0.1)', border: 'none', color: '#A5F3FC', borderRadius: 8, padding: '5px 12px', cursor: 'pointer', fontSize: 12, fontFamily: "Inter, system-ui, sans-serif", marginLeft: 8 }}>
              Déconnexion
            </button>
          </div>
        </div>
      ...
      {/* Hero Banner */}
      <div style={{ background: `linear-gradient(135deg, ${palette.deepPurple} 0%, #0B2A45 60%, ${palette.midPurple} 100%)`, padding: '48px 40px 44px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ display: 'inline-block', background: 'rgba(255,255,255,0.1)', borderRadius: 20, padding: '4px 14px', marginBottom: 14 }}>
            <span style={{ color: '#A5F3FC', fontSize: 12, fontFamily: "Inter, system-ui, sans-serif", letterSpacing: 1.5, textTransform: 'uppercase' }}>Tableau de bord</span>
          </div>
      ...
      <main style={{ maxWidth: 1100, margin: '0 auto', padding: '40px 40px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginBottom: 36 }}>
          ...
          <div style={{ background: palette.card, borderRadius: 16, border: `1px solid ${palette.border}`, padding: '28px 28px', overflow: 'hidden' }}>
            ...
            {loadingGroupe ? (
              <div style={{ color: palette.muted, fontSize: 13, fontFamily: "Inter, system-ui, sans-serif" }}>Chargement...</div>
            ) : monGroupe ? (
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {monGroupe.etudiants?.map((e, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <div>
                        <div style={{ color: palette.text, fontSize: 13, fontFamily: "Inter, system-ui, sans-serif" }}>{e.nom} {e.prenom}</div>
                        <div style={{ color: palette.muted, fontSize: 11, fontFamily: "Inter, system-ui, sans-serif" }}>{e.filiere}</div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div>
                <div style={{ background: palette.warningBg, borderRadius: 10, padding: '14px 16px', marginBottom: 16 }}>
                  <p style={{ margin: '0 0 4px', color: palette.warningText, fontSize: 14, fontFamily: "Inter, system-ui, sans-serif", fontWeight: 500 }}>
                    ⚠️ Vous n'avez pas encore de groupe
                  </p>
                  <p style={{ margin: 0, color: '#B45309', fontSize: 12, fontFamily: "Inter, system-ui, sans-serif" }}>
                ...
                <button
                  ...
                  style={{ width: '100%', background: palette.accent, color: '#fff', border: 'none', borderRadius: 10, padding: '11px', fontSize: 14, fontFamily: "Inter, system-ui, sans-serif", fontWeight: 500, cursor: 'pointer' }}
                >
                  Créer mon groupe →
                </button>
              </div>
            ...
            {loadingAff ? (
              ...
              <div>
                <div style={{ marginBottom: 14 }}>
                  <span style={{ background: affStatut!.bg, color: affStatut!.color, fontSize: 12, fontFamily: "Inter, system-ui, sans-serif", fontWeight: 600, padding: '4px 12px', borderRadius: 20 }}>
                ...
                {monAffectation.projet_titre ? (
                  <div style={{ background: palette.lightPurple, borderRadius: 10, padding: '14px 18px', borderLeft: `3px solid ${palette.accent}` }}>
                    ...
                    {monAffectation.projet_description && (
                      <div style={{ color: palette.muted, fontSize: 12, fontFamily: "Inter, system-ui, sans-serif", lineHeight: 1.5 }}>
                        {monAffectation.projet_description.slice(0, 120)}{monAffectation.projet_description.length > 120 ? '...' : ''}
                      </div>
                    )}
                  </div>
                ) : (
                  <div style={{ color: '#DC2626', fontSize: 13, fontFamily: "Inter, system-ui, sans-serif", background: '#FEF2F2', borderRadius: 10, padding: '12px 16px' }}>
                    Aucun projet disponible assigné
                  </div>
                )}
                <button
                  onClick={() => navigate('/etudiant/resultats')}
                  style={{ marginTop: 14, width: '100%', background: 'transparent', color: palette.accent, border: `1.5px solid ${palette.accent}`, borderRadius: 10, padding: '10px', fontSize: 13, fontFamily: "Inter, system-ui, sans-serif", fontWeight: 500, cursor: 'pointer' }}
                >
              ...
              <div>
                <div style={{ background: '#F3F4F6', borderRadius: 10, padding: '14px 16px', textAlign: 'center' }}>
                  ...
                  <p style={{ margin: 0, color: palette.muted, fontSize: 13, fontFamily: "Inter, system-ui, sans-serif" }}>
                    {!monGroupe
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <p style={{ margin: '0 0 20px', color: palette.muted, fontSize: 14, fontFamily: "Inter, system-ui, sans-serif" }}>
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 18 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 5 }}>
                  <span style={{ background: palette.lightPurple, color: palette.midPurple, fontSize: 10, fontFamily: "Inter, system-ui, sans-serif", fontWeight: 500, padding: '2px 10px', borderRadius: 20 }}>
                    Étape {card.step}
                  </span>
                  <span style={{ background: card.badgeOk ? palette.successBg : palette.warningBg, color: card.badgeOk ? palette.success : palette.warningText, fontSize: 10, fontFamily: "Inter, system-ui, sans-serif", fontWeight: 600, padding: '2px 8px', borderRadius: 20 }}>
                    {card.badge}
                  </span>
                </div>
              </div>
              <h3 style={{ margin: '0 0 8px', color: palette.text, fontSize: 17, fontWeight: 600 }}>{card.title}</h3>
              <p style={{ margin: '0 0 24px', color: palette.muted, fontSize: 13, fontFamily: "Inter, system-ui, sans-serif", lineHeight: 1.6, flexGrow: 1 }}>{card.description}</p>
              <button
                className="cta-btn"
                onClick={() => navigate(card.path)}
                style={{ background: palette.accent, color: '#fff', border: 'none', borderRadius: 10, padding: '11px 0', fontSize: 14, fontFamily: "Inter, system-ui, sans-serif", fontWeight: 500, cursor: 'pointer', width: '100%' }}
              >
                {card.cta} →
              </button>
            </div>
        ...
        <div style={{ marginTop: 32, padding: '18px 22px', borderRadius: 12, background: palette.lightPurple, border: `1px solid ${palette.border}`, display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <p style={{ margin: 0, color: palette.midPurple, fontSize: 13, fontFamily: "Inter, system-ui, sans-serif" }}>
